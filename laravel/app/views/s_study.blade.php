...
<script type="text/javascript">
  ...
  function search_data() {
        ...
        updateValElement();
        $.get('{{ url("student/search_study/search") }}',{currentPage:currentPage },function(data){
            $('#table_study').html(data);
        });
  }
  function updateValElement(){
    condition={word:$('#search-input').val()};
      $.get('{{ url("student/search_study/get_lastpage") }}',{},function(data){
           lastpage=data;
          $('#page_display_1,#page_display_2').html('หน้าที่ '+currentPage+' / '+lastpage)
       });
      $.get('{{ url("student/search_study/get_count") }}',{},function(data){
           count_mem=data;
