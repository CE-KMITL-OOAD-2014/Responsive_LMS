...
<script type="text/javascript">
  ...
  function search_data() {
        ...
        updateValElement();
        $.get('{{ url("teacher/search_absentletter/search") }}',{condition:condition ,currentPage:currentPage },function(data){
  ...
  function updateValElement(){
    condition={approve:$('#show_data_01').prop("checked")?'1':'0',Pending:$('#show_data_02').prop("checked")?'1':'0',unapprove:$('#show_data_03').prop("checked")?'1':'0'};
      $.get('{{ url("teacher/search_absentletter/get_lastpage") }}',{condition:condition},function(data){
           lastpage=data;
          $('#page_display_1,#page_display_2').html('หน้าที่ '+currentPage+' / '+lastpage)
       });
      $.get('{{ url("teacher/search_absentletter/get_count") }}',{condition:condition},function(data){
           count_mem=data;
