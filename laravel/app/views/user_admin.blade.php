...
<div id="wrap"> 
  ...
  <div class="container">
    <div class="page-header">
      <h1>ระบบจัดการAdmin</h1>
    </div>
	
 
    <div class="input-group col-lg-6"> <span class="input-group-addon">ใส่ข้อมูลที่ต้องการค้นหา</span>
      <input type="text" class="form-control" placeholder="ค้นหาจาก = ชื่อ นามสกุล ตำแหน่ง" id="search-input">
    ...
    <div class="panel panel-default" style="background-color:RGBA(255,255,255,0.5); margin-top:20px">
      <div class="panel-body">
        <div class="col-lg-2">
          <div id="outline_count_mem" class="input-group input-group-sm"> <span class="input-group-addon"><span class="glyphicon glyphicon-envelope"></span>&nbsp; จำนวนAdmin</span>
...
<ul class="pager">
    ...
    <ul class="pager">
      ...
      <li id="page_next_2" disabled><a id="a_next_2" href="#" onclick="pageNext();" >หน้าถัดไป <span class="glyphicon glyphicon-chevron-right"></span></a></li>
      <li class="next"><a href="#" onclick="to_last_page()">หน้าสุดท้าย <span class="glyphicon glyphicon-step-forward"></span></a></li>
    </ul>

<div class="line_col12 col-sm-12"></div>
    <div class="col-lg-12 text-center">
      <button class="btn btn-success" onClick="location.href='{{url('admin/add_user_admin')}}';"><span class="glyphicon glyphicon-plus"></span> เพิ่มผู้ใช้งาน</button>
    </div>
<div class="line_col12 col-sm-12"></div>
...
<script type="text/javascript">
  ...
  function search_data() {
        updateValElement();
        $.get('{{ url("search_admin/search") }}',{condition:condition ,currentPage:currentPage },function(data){
  ...
  function updateValElement(){
    condition={word:$('#search-input').val()};
      ...
      $.get('{{ url("search_admin/get_count") }}',{condition:condition},function(data){
           count_mem=data;
